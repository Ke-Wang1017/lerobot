...
- [ ] **`add_features_inplace` UX polish** — confirmation modal for >100k frames (per spec); pre-canned "Common features" preset list in the Add Feature dialog (success/reward/quality_score) so common cases are one click; tri-state colored band rendering on the timeline for pinned per-episode int8 features (today: line plot fallback). (Subtask no longer needs a preset — since 2026-07-10 the dialog special-cases name `subtask` to provision the full LeRobot 3.0 format via `dataset_tools.bootstrap_subtask_format()`: `meta/subtasks.parquet` lookup + per-frame `subtask_index`. See add_feature_review.md N9.)
